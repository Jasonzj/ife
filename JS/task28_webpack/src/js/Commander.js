...
import { Spaceship, SpaceshipGlobal } from './Spaceship'
import { Commander, Planet } from '../app'
...
class God_Commander extends God_Planet {
    constructor() {
        super()
        ;(this.id = 'Jason'), (this.rank = 'admiral'), (this.msgs = []) // 消息队列
    }
}
...
const BUS = (() => {
    ...
    const createSpaceship = msg => {
        ...
        Commander.miniConsole.log(
            `创建飞船${msg.id}成功, 引擎系统为${systemValues[
                msg.engine - 1
            ]},能源系统为${systemValues[msg.energy - 1]}`
        )
    }
...
// 接口
export { God_Commander, God_Planet, BUS }
